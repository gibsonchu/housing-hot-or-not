...
const ADMIN_UNLOCK = 'bt_admin_unlocked';
...
export default function App() {
  ...
  const [adminKey, setAdminKey] = useState(() => {
    try { return sessionStorage.getItem(ADMIN_UNLOCK) || ''; } catch (e) { return ''; }
  });
  ...
  const answeredRef = useRef(answeredQids);
  answeredRef.current = answeredQids;
  ...
  const newPair = useCallback((list) => {
  ...
  const applyState = useCallback((s, repair) => {
    setBuildings(s.buildings);
    buildingsRef.current = s.buildings;
    setTotalVotes(s.totalVotes);
    setUserVotes(s.userVotes);
    setAnsweredQids(s.answeredQids);
    setNeighborhood(s.neighborhood);
    if (repair) newPair(s.buildings);
  }, [newPair]);
  ...
  useEffect(() => {
    let alive = true;
    api.getState()
      .then((s) => { if (alive) { applyState(s, true); setLoading(false); } })
   ...
   */
  const openSurvey = useCallback((votesSoFar) => {
    const answered = answeredRef.current;
    const nth = Math.floor(votesSoFar / SURVEY_EVERY);
    let q = null;
    if (nth % 2 === 0 && !neighborhoodRef.current) {
  ...
  async function answerSurvey(answer) {
    const q = activeQRef.current;
    if (!q) return;
    setActiveQ(null);
    // Pairwise questions with a stable option set feed the community Elo pool.
    let elo = null;
    if (q.elo && typeof answer === 'string') {
      const loser = q.options.find((o) => o !== answer);
      if (loser) elo = { winner: answer, loser };
    }
    setAnsweredQids((prev) => (prev.indexOf(q.id) === -1 ? [...prev, q.id] : prev));
    if (q.type === 'Neighborhood') setNeighborhood(String(answer).trim());
    try {
      await api.sendAnswer(q, answer, elo);
    } catch (e) {
      // The answer is lost, but voting should never be blocked by the survey.
      console.error('Could not save answer:', e.message);
    }
  }
  ...
  const vote = useCallback(async (idx) => {
    ...
    if (!pr || flashRef.current || activeQRef.current || votingRef.current) return;
    ...
    try {
      const r = await api.castVote(winner.id, loser.id);
      const next = bs.map((b) => {
        if (b.id === r.winner.id) return { ...b, rating: r.winner.rating, wins: b.wins + 1 };
        if (b.id === r.loser.id) return { ...b, rating: r.loser.rating, losses: b.losses + 1 };
        return b;
      });
      setBuildings(next);
      ...
      setTimeout(() => {
        ...
        newPair(next);
        if (r.userVotes > 0 && r.userVotes % SURVEY_EVERY === 0) openSurvey(r.userVotes);
      }, 1150);
    } catch (e) {
      votingRef.current = false;
  ...
  useEffect(() => {
    const onKey = (e) => {
      ...
      if (e.key === 'ArrowLeft' || k === 'a') { e.preventDefault(); voteRef.current(0); }
      else if (e.key === 'ArrowRight' || k === 'd') { e.preventDefault(); voteRef.current(1); }
  ...
  const saveBuilding = () => {
    ...
    runAdmin(async () => {
      if (editId) await api.adminUpdate(adminKey, { ...form, id: editId });
      else await api.adminCreate(adminKey, [form]);
      setForm(emptyForm);
      setEditId(null);
    });
  };
  ...
  const deleteBuilding = (id) => runAdmin(() => api.adminDelete(adminKey, id));

  const importBulk = () => {
    const rows = bulk.split('\n').map((l) => l.trim()).filter(Boolean).map((l) => {
      const p = l.split(/[\t|]/).map((s) => s.trim());
      return { address: p[0] || '', neighborhood: p[1] || '', type: p[2] || '', photo: p[3] || '' };
    }).filter((b) => b.address);
    if (!rows.length) return;
    runAdmin(async () => { await api.adminCreate(adminKey, rows); setBulk(''); });
  };

  const resetElo = () => runAdmin(() => api.adminResetElo(adminKey));
  ...
  return (
    <div className={screen === 'vote' ? 'shell shell-lock' : 'shell'} style={{ fontFamily: sans, fontVariantNumeric: 'tabular-nums', color: ink, background: '#fff' }}>
      ...
      {loadError && screen !== 'admin' && (
        <div style={{ background: '#fff6f4', borderBottom: '1px solid #f0d6cf', color: '#8a3a2a', fontSize: 12.5, padding: '10px 22px', textAlign: 'center' }}>
          Couldn&rsquo;t reach the server — {loadError}.{' '}
          <button onClick={() => { setLoadError(null); setLoading(true); reload().catch((e) => setLoadError(e.message)).finally(() => setLoading(false)); }} style={{ background: 'transparent', border: 'none', color: '#8a3a2a', textDecoration: 'underline', cursor: 'pointer', font: 'inherit', padding: 0 }}>Retry</button>
        </div>
      )}
      ...
      {screen === 'vote' && (
        <div className="vote-screen">
          ...
          {left && right ? (
            ...
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 60, textAlign: 'center', fontSize: 14, color: gray }}>
      ...
      {screen === 'profile' && <Profile userVotes={userVotes} onBack={() => setScreen('vote')} />}
          ...
          {detailView && (
            <div style={{ display: 'flex', ...cardBox, marginTop: 20, flexWrap: 'wrap' }}>
              ...
              <div style={{ flex: 1, minWidth: 280, padding: 30 }}>
                ...
                <div style={{ fontFamily: serif, fontSize: 70, fontWeight: 600, margin: '22px 0 0', letterSpacing: '-0.03em', color: ink, lineHeight: 1 }}>{detailView.rating}</div>
                <div style={{ fontSize: 12, letterSpacing: '0.06em', textTransform: 'uppercase', color: gray, marginTop: 6 }}>Current Elo rating</div>
                <div style={{ display: 'flex', marginTop: 26, border: `1px solid ${line}`, borderRadius: 4, overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '60px 28px' }}>
          <form
            onSubmit={async (e) => {
              ...
              setAdminErr('');
              try {
                await api.adminVerify(adminPw);
                setAdminKey(adminPw);
                try { sessionStorage.setItem(ADMIN_UNLOCK, adminPw); } catch (err) {}
                setAdminPw('');
              } catch (err) {
                setAdminErr(err.status === 401 ? 'Wrong password.' : err.message);
              } finally {
                setAdminBusy(false);
              }
            }}
          ...
          >
            ...
            />
            {adminErr && <div style={{ fontSize: 12.5, color: '#b04a3a', marginTop: 10 }}>{adminErr}</div>}
            <button type="submit" disabled={adminBusy} className="btn-green" style={{ ...greenBtn, width: '100%', marginTop: 16, opacity: adminBusy ? 0.5 : 1 }}>{adminBusy ? 'Checking…' : 'Unlock'}</button>
      ...
      {screen === 'admin' && adminOk && (
        <div style={{ flex: 1, padding: '40px 28px', maxWidth: 1000, margin: '0 auto', width: '100%', display: 'flex', flexDirection: 'column', gap: 22 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 12, flexWrap: 'wrap' }}>
            <h1 style={{ fontFamily: serif, fontSize: 'clamp(30px,4vw,42px)', fontWeight: 600, letterSpacing: '-0.02em', color: ink, margin: 0 }}>Admin</h1>
            <button onClick={() => { setAdminKey(''); try { sessionStorage.removeItem(ADMIN_UNLOCK); } catch (e) {} }} className="btn-soft" style={pillBtn}>Lock</button>
          </div>
          {adminErr && <div style={{ border: '1px solid #f0d6cf', background: '#fff6f4', color: '#8a3a2a', padding: '10px 14px', borderRadius: 4, fontSize: 13 }}>{adminErr}</div>}
          ...
          <div style={cardBox}>
            ...
            <div style={{ padding: 20, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              ...
              <label style={labelStyle}>Neighborhood
              ...
              <label style={{ ...labelStyle, gridColumn: '1/-1' }}>Photo URL (optional)
            ...
            <div style={{ padding: '0 20px 20px', display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              <button onClick={saveBuilding} disabled={adminBusy} className="btn-green" style={{ ...greenBtn, opacity: adminBusy ? 0.5 : 1 }}>{editId ? 'Save changes' : 'Add building'}</button>
              {editId && <button onClick={cancelEdit} className="btn-soft" style={softBtn}>Cancel</button>}
            </div>
          </div>
          ...
          <div style={cardBox}>
            ...
            <div style={{ padding: 20 }}>
              ...
              <div style={{ marginTop: 14 }}>
                <button onClick={importBulk} disabled={adminBusy} className="btn-green" style={{ ...greenBtn, opacity: adminBusy ? 0.5 : 1 }}>Import rows</button>
              </div>
            </div>
          </div>
          ...
          <div style={cardBox}>
            ...
            {buildings.map((b, i) => (
              <div key={b.id} style={{ display: 'flex', alignItems: 'center', borderBottom: i === buildings.length - 1 ? 'none' : `1px solid ${line}`, fontSize: 14, gap: 6 }}>
                ...
                <button onClick={() => editBuilding(b)} className="nav-link" style={{ background: 'transparent', color: ink, border: 'none', padding: '13px 12px', cursor: 'pointer', fontSize: 12.5, fontWeight: 600 }}>Edit</button>
                <button onClick={() => deleteBuilding(b.id)} className="nav-link" style={{ background: 'transparent', color: '#b04a3a', border: 'none', padding: '13px 14px', cursor: 'pointer', fontSize: 12.5, fontWeight: 600 }}>Delete</button>
              </div>
            ))}
          </div>
          ...
          <div style={cardBox}>
            <div style={cardHead}>
              ...
              <button onClick={() => copyTsv(exportTsv)} className="btn-soft" style={pillBtn}>{copied ? 'Copied' : 'Copy'}</button>
            </div>
            <div style={{ padding: 20 }}>
              <textarea readOnly value={exportTsv} style={{ ...fieldStyle, height: 120, resize: 'vertical', background: subtle, fontSize: 12.5 }}></textarea>
            </div>
          </div>
      ...
      {activeQ && (
        <SurveyModal
          question={activeQ}
          initialValue={activeQ.type === 'Neighborhood' ? neighborhood : ''}
